...
class CommentCleanScreen(QWidget):
    ...
    def connect_double_click_signals(self):
        ...
        # 连接弹幕双击信号
        self.danmu_viewer.danmu_double_clicked.connect(self.handle_danmu_double_click)
    ...
    @pyqtSlot(int)
    def handle_danmu_double_click(self, dmid: int):
        """处理弹幕双击事件"""
        logger.info(f"弹幕双击: dmid={dmid}")

        try:
            # 直接通过dmid查找弹幕对象
            if dmid not in self.all_danmus:
                logger.warning(f"未找到ID为 {dmid} 的弹幕对象")
                QMessageBox.warning(self, "错误", "未找到对应的弹幕信息")
                return

            danmu = self.all_danmus[dmid]

            from PyQt6.QtGui import QDesktopServices
            from PyQt6.QtCore import QUrl

            if hasattr(danmu, 'source') and danmu.source == "aicu":
                # AICU来源的弹幕，跳转到AICU网站
                ...
                # B站官方来源的弹幕
                if hasattr(danmu, 'video_url') and danmu.video_url:
                    # 使用保存的视频链接（包含dmid）
                    logger.info(f"打开B站视频链接: {danmu.video_url}")
                    QDesktopServices.openUrl(QUrl(danmu.video_url))
                else:
                    # 如果没有video_url，尝试构造链接
                    # 注意：这种情况理论上不应该发生，因为B站弹幕应该都有video_url
                    logger.warning(f"弹幕 {dmid} 没有video_url，尝试其他方式")

                    # 可以尝试通过dmid搜索（虽然不是最佳方案）
                    search_url = f"https://www.bilibili.com/video/BV1rJNjzGEWY?dm_progress=7474&p=1&dmid={dmid}"
                    logger.info(f"使用dmid搜索: {search_url}")
                    QDesktopServices.openUrl(QUrl(search_url))
    ...
    def start_incremental_fetch(self):
        ...
        self.incremental_fetch_thread = IncrementalFetchThread(self.api_service, self.db_manager)
    ...
    def start_incremental_fetch(self):
        """开始增量获取"""
        if hasattr(self, 'incremental_fetch_thread') and self.incremental_fetch_thread.isRunning():
            return
...
# 这个MainWindow类已经不再需要了，因为我们现在直接使用ImprovedToolSelectionScreen
# 但为了保持向后兼容性，我们暂时保留它
...
class IncrementalFetchThread(QThread):
        ...
        def __init__(self, api_service, db_manager):
            super().__init__()
            self.api_service = api_service
        ...
        async def _simple_incremental_fetch(self, api_service, uid, progress_callback):
            ...
            try:
                ...
                with self.db_manager.get_connection() as conn:
                    ...
                    # 获取最新的通知时间戳
                    cursor.execute('''
                        SELECT MAX(created_time) FROM notifies WHERE uid = ?
                    ''', (uid,))
                    result = cursor.fetchone()
                    last_notify_time = result[0] if result[0] else 0
                ...
                logger.info(
                    f"数据库最新时间戳 - 通知: {last_notify_time}, 评论: {last_comment_time}, 弹幕: {last_danmu_time}")

                # 1. 获取新的点赞通知
                if self._is_running:
                    progress_callback("获取新的点赞数据...")
                    await self._fetch_liked_incremental(api_service, uid, last_notify_time, new_notifies, new_comments,
                                                        new_danmus)

                # 2. 获取新的回复通知
                if self._is_running:
                    progress_callback("获取新的回复数据...")
                    await self._fetch_replied_incremental(api_service, uid, last_notify_time, new_notifies,
                                                          new_comments)

                # 3. 获取新的@通知
                if self._is_running:
                    progress_callback("获取新的@数据...")
                    await self._fetch_ated_incremental(api_service, uid, last_notify_time, new_notifies)

                logger.info(f"增量获取完成: 通知 {len(new_notifies)}, 评论 {len(new_comments)}, 弹幕 {len(new_danmus)}")
        ...
        async def _fetch_liked_incremental(self, api_service, uid, last_time, new_notifies, new_comments, new_danmus):
            """获取新的点赞数据"""
            try:
                    ...
                    if item_data.get("type") == "reply":
                        ...
                        if rpid:
                            try:
                                ...
                                new_comments[rpid] = Comment.new_with_notify(
                                    oid=oid, type=type_, content=content, notify_id=notify_id, tp=0
                                )
                                new_comments[rpid].created_time = like_time
                            except Exception as e:
        ...
        async def _fetch_replied_incremental(self, api_service, uid, last_time, new_notifies, new_comments):
            """获取新的回复数据"""
            try:
                    ...
                    if item_data.get("type") == "reply":
                        ...
                        if rpid:
                            try:
                                ...
                                new_comments[rpid].created_time = reply_time
                            except Exception as e:
